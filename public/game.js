...
let myId=null, isHost=false, localTimer=null, selectedVoteId=null, isMyPlayerDead = false;
...
document.getElementById('btnCreateRoom').onclick=()=>{
    ...
    // IMPORTANTE: Enviar el nombre correctamente
...
btnSkip.onclick=()=>{ 
    if(isMyPlayerDead) return; // Muertos no votan
...
socket.on('votingResults', (data)=>{
    ...
    if(data.kickedPlayer){ 
        ...
        t.innerText="Nadie fue expulsado."; s.innerText="SKIP / EMPATE"; s.className="eject-subtitle"; 
    }
...
function renderSidebar(room){
    ...
    room.players.forEach(p=>{
        ...
        // Estilo hablando (solo si no esta muerto)
        if(!p.isDead && room.phase==='palabras'&&room.players[room.turnIndex]?.id===p.id) c.classList.add('talking');
...
function updateGameView(room){
    // Ocultar sidebar si es votación
...
function renderVotingGrid(players){
    ...
    players.forEach(p=>{
        // NO MOSTRAR MUERTOS EN LA VOTACIÓN
        if(p.isDead) return;
...
function updateHeader(room){ 
    ...
    if(room.phase==='lobby'){ 
        btnStart.style.display='block'; btnSkip.style.display='none'; 
        if(isHost&&room.players.length>=3){btnStart.disabled=false; btnStart.style.opacity='1';}
        else{btnStart.disabled=true; btnStart.style.opacity='0.5';} 
    }else if(room.phase==='votacion'){ 
        btnStart.style.display='none'; btnSkip.style.display='block'; 
        // Si estoy muerto, deshabilito el botón de saltar
        if(isMyPlayerDead) { btnSkip.innerText = "ESTÁS MUERTO"; btnSkip.disabled = true; }
    }else{ 
        btnStart.style.display='block'; btnStart.disabled=true; btnSkip.style.display='none'; 
    } 
}
